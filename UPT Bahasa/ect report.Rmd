...
tabel2 <- tabel1 %>% filter(!is.na(jenis_kelamin), !is.na(skor), skor != 0)
...
```{r}
# Cek Apakah ada Jenis Kelamin yang belum diisi

#tabel1 %>% filter(skor > 0 & is.na(jenis_kelamin))

# Cek Apakah ada nilai L1 yang belum diisi

#tabel1 %>% filter(is.na(l1) & !is.na(g1))

# Cek apakah ada yang belum mengisi tempat tanggal lahir

#tabel2 %>% filter((is.na(tempat_lahir) | is.na(tanggal_lahir)) & skor > 0 )

# Cek salah tulis jenis kelamin
tabel2 %>%  filter(jenis_kelamin != "Laki-laki", jenis_kelamin != "Perempuan")
```
