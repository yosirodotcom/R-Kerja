...
tabel2 <- tabel %>% filter(ISO >= 154) 


```

```{r}
tabel3 <- left_join(tabel2, tabel1, by = "nim", suffix = c(".2", ".1")) 
tabel3 <- tabel3 %>% select(Prodi = Prodi.2, Semester = semester.2, Kelas = kelas.2, Nama = nama.2, NIM = nim, `Skor Awal` = skor.1, `Skor Akhir` = skor.2) %>% arrange(Prodi, Kelas, Nama) 
```

```{r}
jumlah_peserta <- tabel3 %>% 
        filter(!is.na(`Skor Akhir`)) %>% 
        group_by(Prodi, Semester, Kelas) %>% 
        summarize(Jumlah = n())

total <- sum(jumlah_peserta$Jumlah) 
```

Total peserta yang mengikuti : `r total`
